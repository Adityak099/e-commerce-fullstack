...
export default function AddToCartButton({ product, className = "" }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setFeedbackMessage(
        error.response?.data?.error || error.message || "Could not add item",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-2">
      {quantity > 0 ? (
        <div
          className={`flex items-center justify-between rounded-full bg-[#1f3c88] px-3 py-2 text-white ${className}`}
        >
          <button
            type="button"
