  ...
  return (
    <article className="overflow-hidden rounded-[30px] border border-white/70 bg-[rgba(255,253,248,0.88)] shadow-[0_18px_45px_rgba(15,23,42,0.06)]">
      <Link href={`/products/${product.slug}`} className="block">
        <div className="aspect-[4/4.3] overflow-hidden bg-[#f3ead8] relative">
          <Image
            fill
            src={product.image}
            alt={product.name}
      ...
      <div className="space-y-4 p-5">
        ...
        <p className="line-clamp-2 text-sm leading-6 text-[#52606d]">
          {product.description}
        </p>

        <div className="flex items-center justify-between gap-4">
          <p className="text-2xl font-semibold text-[#1f3c88]">
